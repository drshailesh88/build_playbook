...
| `ralph/qa.sh` | Verify loop. Invokes Codex with dual-account failover. Picks first `passes:true` feature not yet `qa_tested:true`. Fixes bugs in code (never tests), commits with `QA:` prefix, flips `qa_tested:true`. Runs the full judge ladder (T0+T1+T2) on each candidate BEFORE spending grader tokens; rejected stories return to the build loop. |
| `ralph/judge.sh` | **Deterministic-first verdict ladder (DEC-004).** T0 invariants (contract hash, story commit exists, locked paths, forbidden patterns from `t0-rules.jsonl`) → T1 mechanical proof (tsc, lint, the story's fail_to_pass tests, test-count guard, optional mutation guard) → T2 semantic LLM judge (independent context, READ-ONLY tools, completeness + decision-trace audit). Writes `ralph/verdicts/<story-id>.json`. Exit 0 PASS / 1 FAIL / 2 ESCALATE. Env: `JUDGE_TIERS`, `JUDGE_T2_MODEL`, `JUDGE_T1_FULL_SUITE`, `JUDGE_T1_MUTATION`. |
| `ralph/freeze-contracts.sh` | **Frozen story contracts (DEC-005).** Compiles each prd.json story into `ralph/contracts/<id>.contract.md` BEFORE building, records sha256 in `manifest.sha256`, commits contracts in their own `FREEZE:` commit. Append-only — never regenerates a frozen contract. Refreeze requires human sign-off: `FREEZE_ACK=<id> ./ralph/freeze-contracts.sh --refreeze <id>`. Judge T0 fails any story whose contract hash changed mid-run. |
| `ralph/t0-rules.template.jsonl` | T0 invariant rules: locked-path and forbid-pattern rules (secrets, `.only()` tests, skip-without-issue, locked configs/hooks/contracts). **This is the ratchet's landing zone** — when the T2 judge or a reviewer catches the same class of mistake twice, append a rule here so it can never recur. Rename to `t0-rules.jsonl`. |
| `ralph/judge-t2-prompt.template.md` | T2 semantic judge instructions: completeness vs the frozen contract + decision-trace audit (flags novel architecture decisions the contract didn't authorize → ESCALATE). Rename to `judge-t2-prompt.md` and fill the Project Lens section. |
| `ralph/review-quorum.sh` | **Two-reviewer quorum (DEC-003/009).** Codex (runtime skeptic, dual-account failover) and Grok (scope minimalist) review a story in parallel as READ-ONLY independent reviewers. 2-of-2 semantics: both-agree findings = confirmed (auto-fixable), single-reviewer findings = human triage queue, verdict disagreement = ESCALATE (never auto-merge on conflict). `--all` quorums every qa_tested story without a verdict. Writes `ralph/reviews/<id>.quorum.json`; rejections feed `progress.txt`. Runs standalone until runtime-proven, then chain into run.sh after QA. |
| `ralph/reviewer-codex-prompt.template.md` | Codex runtime-skeptic lens: seven attack surfaces (auth, data loss, rollbacks, races, degraded dependencies, version skew, observability gaps). Evidence-anchored findings only. Rename to `reviewer-codex-prompt.md`. |
| `ralph/reviewer-grok-prompt.template.md` | Grok scope-minimalist lens: completeness vs frozen contract, scope-expansion hunt, decision-trace audit against the DEC ledger. Rename to `reviewer-grok-prompt.md`. |
| `ralph/drift-audit.sh` | **Aggregate-drift audit (DEC-009 item 5).** `snapshot` at run start records deps + sha256 of schema/config surfaces; `audit` at run end attributes every change to a story whose frozen contract authorizes it. Unattributed changes (a smuggled dependency, a non-story config edit, schema work no contract mentions) → `ralph/drift-report.json` + tracking-issue note, reviewed in `/morning-review`. Catches cross-story accumulation no per-story check can see. |
| `ralph/witness.sh` | **Liveness watchdog (DEC-009 item 4).** Runs OUTSIDE the loop (cron / ralph-witness.timer, ~10 min). Catches what circuit breakers can't: loops alive but stuck. Stale heartbeat + live loop = STALLED (notify; `WITNESS_AUTOKILL=1` kills the stuck agent process scoped to this project's cwd); stale + dead + cursor not idle = CRASHED (notify with resume instructions). Also disk >90% and budget-overdue checks. |
| `ralph/systemd/` | systemd user units: `ralph.service` (runs run.sh; `Restart=on-failure` with `RestartPreventExitStatus=2 3 4` — crashes restart, deliberate stops [ABORT/breaker/budget] stay stopped; `MemoryMax` guard) + `ralph-witness.service`/`.timer` (10-min watchdog). Install steps in the unit comments; remember `loginctl enable-linger`. |
| `ralph/gh-state.sh` | **GitHub as source of truth (DEC-004 Phase 3).** `init` creates one issue per story (frozen contract in body, status as `s:pending/built/qa/approved` labels) + a pinned tracking issue holding the crash-recovery cursor. Loops push events (label swap + verdict-JSON comment trail) and pull state back (`pull` — GitHub wins, so relabeling an issue from your phone re-queues or parks a story). `escalate` = park + `escalated` label + Slack webhook (`NOTIFY_WEBHOOK`); the factory skips parked stories and never blocks on a human. `recover` prints the cursor + parked list after a crash. `GH_STATE=0` degrades everything to local files. |
...
| `ralph/run.sh` | Master entrypoint. Freezes contracts, then chains `build.sh` ↔ `qa.sh` in a bounce loop (judge-rejected stories return to build, `RALPH_MAX_BOUNCES` rounds, default 1) → `harden.sh`. Env skips: `BUILD_SKIP=1` `QA_SKIP=1` `HARDEN_SKIP=1`. Logs to `ralph/ralph-<timestamp>.log`. macOS notification + opens `progress.txt` on completion. Other harden loops run standalone until runtime-proven, then get chained in here. |
