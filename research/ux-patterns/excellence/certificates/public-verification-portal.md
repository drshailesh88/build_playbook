...
2. **Manual lookup portal** for typed IDs: Sertifier is the only one OBSERVED live (verified.sertifier.com/en/search); its not-found verdict — "Oops! Sorry, there is no user matching with the given username." + "Report an issue" — means a typo gets a neutral not-found, never a fraud accusation. Accredible has NO public type-a-code portal (accredible.com/verification 301-redirects to a help article — raw notes; its model is per-credential URL + in-page Verify only). PMI runs a registry (bot-blocked, unobserved).
