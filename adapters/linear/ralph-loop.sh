...
Run: ${TEST_CMD} && ${TYPE_CHECK_CMD}" \
    ...
    if [ "$HEALED" = false ]; then
      ...
      # Revert all changes on current branch
      git checkout -- . 2>/dev/null || true
      git clean -fd 2>/dev/null || true

      linear issue update "$ISSUE_ID" --state "Blocked" 2>/dev/null || true
      linear issue comment add "$ISSUE_ID" --body "**Ralph Loop** — BUILD FAILED. Score dropped ${BEFORE_PASS}→${AFTER_PASS}, could not heal after ${MAX_HEAL_ATTEMPTS} attempts. Changes reverted. Needs manual attention." 2>/dev/null || true
...
Ralph Loop iteration ${i}. Tests: ${BEFORE_PASS} → ${AFTER_PASS} (+$((AFTER_PASS - BEFORE_PASS)))."
  ...
  linear issue comment add "$ISSUE_ID" --body "**Ralph Loop** — DONE. Tests: ${BEFORE_PASS} → ${AFTER_PASS} (+$((AFTER_PASS - BEFORE_PASS))). Iteration ${i}." 2>/dev/null || true
...
echo -e ""
echo -e "Review:  ${YELLOW}git log --oneline -${COMPLETED}${NC}"
echo -e "Next:    ${YELLOW}/playbook:anneal${NC} to stress-test, or ${YELLOW}/playbook:verify-with-codex${NC} for adversarial review"
